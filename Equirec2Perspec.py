...
#        print("Passo  8: %.4f"%(time.time() - start))
        ...
        R1, t = cv2.Rodrigues((y_axis * np.radians(THETA)).get())
        R2, t = cv2.Rodrigues( (np.dot( np.asarray(R1), x_axis) * np.radians(PHI)).get()  )
        R = R2 @ R1
...
#        print("Passo 11: %.4f"%(time.time() - start))
        ...
        persp = cv2.remap(self._img, (XY[..., 0]).get(), (XY[..., 1]).get(), cv2.INTER_LINEAR, borderMode=cv2.BORDER_WRAP)
#        print(time.time() - start)
...
#for i in range(10):
#    start = time.time()
#    img = e.GetPerspective( 85, 0, 0, 720, 1024)
#    print(time.time() - start)
#    print("Fim")
...
while angle < 90:
    ...
    img = e.GetPerspective( 85, angle, 0, 720, 1024)
